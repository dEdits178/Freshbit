...
async function testUnauthorizedCollege() {
  try {
    // Get the OTHER college user (college2) - not invited to this drive
    const college = await prisma.user.findUnique({
      where: { email: 'college2@freshbit.test' }
    });

    if (!college) {
      console.error('College2 user not found');
      return;
    }

    // Generate token for college2
    ...
    console.log('Using college2 (not invited to test-drive-1)');
